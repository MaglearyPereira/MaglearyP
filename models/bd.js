...
const transporter = nodemailer.createTransport({
  service:'Gmail',
  auth:{
    user:'maglearypereirao@gmail.com',
    pass:'nniayhfomopmmuaj'
  }
});
...
function registroUsuariosPost(req,res){
 ...
 if(e) return console.error(e.message);
  ...
  const mensaje = {
  from:'maglearypereirao@gmail.com',
  to:email,
  subject:'!Bienvenido a nuestra página web¡',
  text:`Hola ${nombre} , !Te damos la Bienvenida a una de las mejores páginas de venta y compra de Linea Blanca, donde podras encontrar gran variedad de productos y marcas¡`
  }
...
db.run(sql,[nombre_de_usuario,apellido,cedula,telefono,cliente_id,producto_id,cantidad,total_pagado,ip_cliente,codigo,numeroT,cvv,mesV,year,currency,descripcion],(err)=>{
  ...
  if(err) return console.error(err.message);
  const mailOptions = {
    from: 'maglearypereirao@gmail.com', // Reemplaza con tu dirección de correo electrónico
...
function puntuaciones(req,res){
  ...
  // Generar un token único
   ...
   res.cookie('securityToken',token, { httpOnly: true, secure: true });
    ...
    const recoveryURL = `https://lineablanca-mbqk.onrender.com/restablecer-contrasena?token=${token}?userName=${UserName}`;
